...
st.write("각 문항에 대해 특징을 보고 올바른 원소 이름을 골라보세요.")
...
if st.session_state.current_q < 10:
    ...
    selected = st.radio("어떤 원소일까요?", choices, key=f"q{st.session_state.current_q}")

    if st.button("제출하기", key=f"submit{st.session_state.current_q}"):
        if selected == q["name"]:
            st.success("정답입니다! 🎉")
